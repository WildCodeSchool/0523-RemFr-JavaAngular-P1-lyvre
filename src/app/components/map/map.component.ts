    ...
    const showMarker = (initialMarkers: RecordFields[]) => {
      for (let i = 0; i < initialMarkers.length; i++) {
        ...
        if(initialMarkers[i].photo){
          contenuPopup += "<br><img class=\"custom-popup leaflet-popup-content-img\" width= 40%; src=\"" + initialMarkers[i].photo + "\">";
        } else {
          contenuPopup += "<br><img class=\"custom-popup leaflet-popup-content-img\" width=40% src=\"../assets/bal-default_image.png\">";
        }

        contenuPopup += "<div class=\"text-popup\">" + initialMarkers[i].nom + "</div><br>" + "<div class=\"text-popup-italic\">" + initialMarkers[i].adresse + " " + initialMarkers[i].code_postal + " " + initialMarkers[i].commune + "</div>";
        
        if(initialMarkers[i].gestionnaire){
          contenuPopup += "<div class=\"text-popup-italic\"><br>" + "Gestionnaire : " + initialMarkers[i].gestionnaire + "</div>";
        } else {
          contenuPopup += "";
        }
